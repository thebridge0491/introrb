...
def render_skeleton(skeleton='skeleton-rb', ctx=None):
	...
	renderouts, copyouts, pat_liquid = {}, {}, re.compile('\.liquid$')
	envPaths = Environment()
	for skelX in files_skel:
		template = envPaths.from_string(os.path.join(CUR_DIR, ctx['name'], skelX))
		if pat_liquid.search(skelX) :
			renderouts[skelX] = re.sub(pat_liquid, '', template.render(ctx))
		else:
			copyouts[skelX] = template.render(ctx)
	...
	envOuts = Environment(loader=FileSystemLoader(start_dir))
...
def parse_cmdopts(args=None):
	...
	opts_parser.add_argument('template', nargs='?', default='skeleton-rb',
		help='Template path')
...
def main(argv=None):
	...
	switcher = {
		...
		False: lambda: opts_hash.fileOut.write(Environment(
			loader=FileSystemLoader(CUR_DIR)).get_template(
