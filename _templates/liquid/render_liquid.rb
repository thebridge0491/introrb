...
module RenderLiquid
	...
	def render_skeleton(skeleton='skeleton-rb', ctx={})
		...
		keys2str = proc {|h| h.map{|k, v|
		  [k.to_s, v.is_a?(Hash) ? keys2str.call(v) : v]}.to_h}
		renderouts, copyouts, pat_liquid = {}, {}, /\.liquid$/
		files_skel.each{|skelX|
			template = Liquid::Template.parse(skelX)
			if pat_liquid.match(skelX)
				renderouts[skelX] = template.render(keys2str.call ctx).sub(pat_liquid, '')
			else
				copyouts[skelX] = template.render(keys2str.call ctx)
			end
			}
		...
		renderouts.each{|srcR, dstR| File.open(File.join(ctx[:name], dstR), 'w+') {|f|
			template = Liquid::Template.parse(File.read(File.join(start_dir, srcR)))
			f.write(template.render(keys2str.call ctx))
			}
		}
...
Example: #{File.basename $PROGRAM_NAME} -d data.yaml LICENSE.liquid
EOF
	...
	def main(args=[])
	  ...
	  if not is_dir
	    ...
	    cfg = cfg.merge(kvset)
	    template = Liquid::Template.parse(File.read(opts_hash[:template]))
			opts_hash[:fileOut].write(template.render(keys2str.call cfg))
	  else
	    if not '-' == opts_hash[:data]
